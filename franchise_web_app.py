...
MAP_FILE    = "industry to business type.xlsx"   # <‑‑ put this in the same folder
RESULT_LIMIT = 10
# ----------------------------
...
# Expecting two columns:  business_type | industry
...
# build dict  {business_type: [industry1, industry2, ...]}
biz_map = (
    map_df.groupby("business_type")["industry"]
    .apply(list)
    .to_dict()
)
...
work_setting = st.selectbox(
    ...
    ["Please select", "Mostly outdoors / in-field",
     "Office / professional", "Home-based / mobile"],
)
...
if st.button("Find My Matches 🚀"):
    ...
    if "Please select" in [liquid_capital, hands_on_time,
                           work_setting, customer_type]:
        st.warning("Please complete the remaining dropdowns.")
        st.stop()

    # ============== FILTER PHASE 1: BUSINESS‑FOCUS (highest priority) ==============
    def row_matches_focus(industry_cell: str) -> bool:
        """
        Returns True if the row's Industry list fulfills *all*
        selected business‑focus categories.
        """
        inds = [i.strip().lower() for i in str(industry_cell).split(",")]
        for bf in biz_focus:
            needed = [x.lower() for x in biz_map[bf]]
            if not any(n in ind for n in needed for ind in inds):
                return False
        return True

    df_f = df[df["industry"].apply(row_matches_focus)]

    # If nothing survives, stop right away (no point applying other filters)
    if df_f.empty:
        st.error("No franchises matched all selected Business‑Focus categories.")
        st.stop()

    # ============== FILTER PHASE 2: FINANCIAL & OTHER CRITERIA ==============
    ...
    if work_setting == "Mostly outdoors / in-field":
        df_f = df_f[df_f["industry"].str.contains(
            "home services|repair|restoration|lawn|pest",
            case=False, na=False)]
    elif work_setting == "Office / professional":
        df_f = df_f[df_f["industry"].str.contains(
            "advertising|real estate|business services",
            case=False, na=False)]
    elif work_setting == "Home-based / mobile":
        df_f = df_f[df_f["home based franchise"] == "Yes"]

    # optional industry tags from previous question
    ...
    # Fallback—still enforce Business‑Focus, ignore financials
    if df_f.empty:
        st.info("No brand met all extra filters. Showing top‑ranked brands that fit "
                "your Business‑Focus categories instead.")
        df_f = df[df["industry"].apply(row_matches_focus)].sort_values("industry_ranking")

    top_n = df_f.head(RESULT_LIMIT)

    if top_n.empty:
        st.error("No franchises to display — consider broadening your answers.")
        st.stop()
    ...
    for _, row in top_n.iterrows():
        ...
        st.markdown(f"**Startup Cost:** {clean_money(row['cash required'])}")
        st.markdown(f"**Franchise Fee:** {clean_money(val('franchise fee - one unit'))}")
